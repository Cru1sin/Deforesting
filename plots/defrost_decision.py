...
def _style(algorithm: str) -> tuple[str, str, str]:
    base, separator, run_name = algorithm.partition("__")
    color, marker, label = V26_PATCH_STYLES[base] if base in V26_PATCH_STYLES else STYLES[base]
    suffix = "diagnostic minimum" if base == "v2.6.8" else "optimum"
    return color, marker, f"{base.upper()} ({run_name}) {suffix}" if separator else label
...
def _run_key(recipe: Mapping[str, object]) -> tuple[str, str]:
    ...
    run_name = recipe.get("run_name")
    key = base_cost if run_name in (None, "") else f"{base_cost}__{run_name}"
    label = base_cost.upper() if run_name in (None, "") else f"{base_cost.upper()} ({run_name})"
    return key, label
...
def _load_result_tables(
    ...
    for directory in result_dirs:
        ...
        for _cycle_name, curve in table.groupby("cycle_name", sort=False):
            ...
            if "selected" in curve and "selected_defrost_time" in curve:
                selected = curve.loc[curve["selected"].fillna(False).astype(bool)]
                selected_defrost_times = (
                    pd.to_datetime(curve["selected_defrost_time"], errors="coerce", format="mixed")
                    .dropna()
                    .drop_duplicates()
                )
                if selected.empty and selected_defrost_times.empty:
                ...
                candidate_defrost_time = pd.to_datetime(
                    selected["candidate_defrost_time"], errors="coerce", format="mixed"
                )
                if (
                    len(selected) != 1
                    or len(selected_defrost_times) != 1
                    or pd.isna(candidate_defrost_time.iloc[0])
                    or candidate_defrost_time.iloc[0] != selected_defrost_times.iloc[0]
                ):
                    raise ValueError(f"{_cycle_name}: invalid stored selection")
                table.loc[curve.index, "t_star"] = candidate_defrost_time.iloc[0]
                table.loc[curve.index, "t_star_model_supported"] = selected.iloc[0].get(
                    "selected_point_in_training_domain", pd.NA
                )
                continue
            if base == "cop_heating_rate_pareto_knee":
                selected_defrost_times = (
                    pd.to_datetime(
                        curve.get("selected_defrost_time", pd.Series(dtype=object)),
                        errors="coerce",
                        format="mixed",
                    )
                    .dropna()
                ...
                if (
                    ...
                    raise ValueError(f"{_cycle_name}: invalid COP–heating-rate Pareto selection")
            ...
            if base == "v2.6.8":
                ...
                selected = curve.loc[
                    ...
                    ).eq(minimum.iloc[0] if not minimum.empty else pd.NaT)
                ]
            ...
            table.loc[curve.index, "t_star"] = pd.to_datetime(
                selected_row["candidate_defrost_time"], errors="coerce", format="mixed"
            )
...
def _cycle_points(table: pd.DataFrame, optimum_column: str = "t_star") -> pd.DataFrame:
    values = table.copy()
    for column in ("candidate_defrost_time", "cycle_start", optimum_column, "t_RB"):
    ...
    for offset, algorithm in zip(offsets, algorithms, strict=True):
        ...
        if base in STATUS_MARKERS:
            ...
            if unknown or values["cycle_status"].isna().any():
                raise ValueError(f"unrecognized {base.upper()} cycle_status: {sorted(unknown)}")
        ...
        if no_minimum.any():
            missing_label = (
                "COP–heating-rate Pareto selection (no selected point)"
                if base == "cop_heating_rate_pareto_knee"
...
def _cost_curve_figure(
    ...
    for algorithm, table in tables.items():
        ...
        minutes = (curve["candidate_defrost_time"] - start).dt.total_seconds() / 60
        ...
        linestyle = CURVE_LINESTYLES.get(algorithm.split("__", 1)[0], CURVE_LINESTYLES["v2.6"])
        cost_axis.plot(
            minutes,
            cost,
            color=color,
        ...
        if pd.notna(target):
            optimum = (curve["candidate_defrost_time"] - target).abs().idxmin()
    ...
    status_algorithm = next(
        (algorithm for algorithm in tables if algorithm.split("__", 1)[0] in {"v2.6.7", "v2.6.6"}),
        None,
    )
...
def _match_optimal_front_images(
    ...
    for algorithm, table in tables.items():
        ...
        matched[algorithm]["target_status"] = (
            cycle_status
            if algorithm.split("__", 1)[0] in STATUS_MARKERS and cycle_status != "identified_curve"
            else ""
        )
    return matched
    ...
    for cycle_name in cycles:
        ...
        for label, (table, title) in suites.items():
            ...
            render_decision_publication(
                ...
                optimal_label=(
                    "COP–heating-rate Pareto knee ("
                    f"{str(curve.iloc[0].get('selection_reason', 'unknown')).replace('_', ' ')}"
                    ")"
                    if algorithm == "cop_heating_rate_pareto_knee"
...
def render_current_decision_figures(
    ...
    table["t_star"] = pd.NaT
    table["t_star_model_supported"] = pd.NA
    for _cycle_name, curve in table.groupby("cycle_name", sort=False):
        selected = curve.loc[curve["is_selected"].fillna(False).astype(bool)]
        if selected.empty:
            continue
        row = selected.iloc[0]
        table.loc[curve.index, "t_star"] = pd.Timestamp(row["candidate_defrost_time"])
        table.loc[curve.index, "t_star_model_supported"] = row["selected_point_in_training_domain"]
    records = {
...
def _plot_ticket_loeo(loeo: pd.DataFrame, target: str) -> plt.Figure:
    ...
    if missing:
        raise ValueError(f"defrost-event LOEO audit missing columns: {sorted(missing)}")
    values = loeo.loc[loeo["target"].eq(target)].copy()
    if values.empty:
        raise ValueError(f"defrost-event LOEO audit has no {target} rows")
