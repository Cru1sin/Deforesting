"""Plot the publication evidence linking cost regret to RGB image labels."""
...
def high_confidence_coverage(
    ...
    rows = label_balance.loc[
        ...
        & label_balance["timing_state"].isin(
            ("before_reference", "near_reference", "after_reference")
        )
    ]
    retained = rows.loc[rows["timing_state"].ne("near_reference"), "image_count"].sum()
    return float(retained / rows["image_count"].sum())


def regret_threshold_summary(bands: pd.DataFrame, label_balance: pd.DataFrame) -> pd.DataFrame:
...
def _band_widths(cost: pd.DataFrame, thresholds: Sequence[float]) -> pd.DataFrame:
    ...
    eligible["candidate_defrost_time"] = pd.to_datetime(
        eligible["candidate_defrost_time"], errors="coerce", format="mixed"
    )
            ...
            rows.append(
                {
                    ...
                    "band_width_minutes": (
                        curve["candidate_defrost_time"].max()
                        - curve["candidate_defrost_time"].min()
                    ).total_seconds()
                    / 60,
                }
            )
...
def plot_label_figures(
    *,
    ...
    curve["minutes"] = (
        curve["candidate_defrost_time"] - curve["candidate_defrost_time"].min()
    ).dt.total_seconds() / 60
    optimum = curve.loc[curve["inverse_cop"].idxmin(), "candidate_defrost_time"]
    curve["state_01pct"] = np.where(
        curve["relative_regret"].le(0.01),
        "near-optimal",
        np.where(curve["candidate_defrost_time"].lt(optimum), "pre-optimal", "post-optimal"),
    )
