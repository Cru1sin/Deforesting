...
def build_cycle_slices(
    ...
    for key in keys:
        ...
        grid = (
            expected_grid(start, end, interval_seconds)
            if start and end
...
def _future_record(  # noqa: C901
    ...
    valid_feature = (
        ...
        else pd.Series(False, index=grid)
    )
...
def _resolve_cycle_reference(
    ...
    return resolve_analysis_reference(
        ...
        configured_baseline_available=(
            baseline_status == "available" and residual_name in cycle.frame
        ),
...
def _global_spearman(
    ...
    if (
        expected == 0
        or count < policy.min_segment_points
        or count / expected < policy.min_segment_coverage
    ):
        return np.nan
    x = pd.Series(
...
def _cycle_status_reason(summary: pd.Series, frame: pd.DataFrame) -> str | None:
    value: Any = (
        summary.get("cycle_status_reason")
        if "cycle_status_reason" in summary.index
        else None
    )
    if (
        value is None
    ...
    ) and "cycle_status_reason" in frame and not frame.empty:
