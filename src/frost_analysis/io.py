...
def _load_evidence_run(  # noqa: C901
    run_dir: Path,
    registry_hash: str,
    ...
    if (
        ...
        raise FileNotFoundError(
            "evidence run must contain manifest.json, processed_data.parquet "
            f"and cycle_summary.csv: {run_dir}"
        )
    ...
    if not experiment_id or not is_iso_date(experiment_date):
        raise ValueError(
            "manifest must contain experiment_id and ISO experiment_date: "
            f"{run_dir}"
        )
