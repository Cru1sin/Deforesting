...
from image_labels.timing import validate_cost
...
def test_gate_rejects_missing_columns_clearly() -> None:
    cost = _canonical_cost().drop(columns="candidate_defrost_time")

    with pytest.raises(ValueError, match="missing required columns: candidate_defrost_time"):
        validate_cost(cost)
