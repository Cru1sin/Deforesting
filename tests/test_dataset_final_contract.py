...
def test_collect_cycle_images_uses_cycle_window_without_sensor_match(
    ...
    from frost_analysis.dataset_images import collect_cycle_images
...
def test_image_metadata_has_no_sensor_alignment_fields() -> None:
    from frost_analysis.dataset_images import image_metadata_frame
...
def test_materialize_cycle_images_downloads_default_cloud_zip_with_rclone(
    ...
    cloud = tmp_path / "OneDrive" / "images"
    cloud.mkdir(parents=True)
    (cloud / f"{cycle_name}.zip").write_bytes(b"cloud placeholder")
    monkeypatch.setattr(dataset_images, "DEFAULT_CLOUD_IMAGES_ROOT", cloud)
...
def test_build_date_prints_processing_stages(
    ...
    from frost_analysis import channels, prepare, process, validation
    from frost_analysis.dataset import _build_date
...
def test_render_can_explicitly_fetch_cloud_cycle_images(
    ...
    monkeypatch.setattr(dataset_images, "DEFAULT_CLOUD_IMAGES_ROOT", cloud)
    monkeypatch.setattr(
        dataset_images.subprocess,
        "run",
        lambda command, *, check: shutil.copyfile(archive, Path(command[3])),
    )
    seen: list[list[Path]] = []
    monkeypatch.setattr(
        visualization,
...
def test_cycle_assets_has_publication_and_panel_without_coverage() -> None:
    from frost_analysis.dataset_metadata import cycle_assets
...
def test_refresh_dataset_does_not_modify_manifest(
    ...
    import frost_analysis.dataset_validation as validation
...
def test_dataset_remove_deletes_only_selected_experiment(
    ...
    import frost_analysis.dataset_validation as validation
    from frost_analysis.dataset import remove_dataset
...
def test_rgb_stage_metrics_require_all_expected_roles() -> None:
    from frost_analysis.dataset_images import rgb_stage_metrics
...
def test_aggregate_original_restores_10s_and_builds_30s_with_new_channel(
    ...
    import frost_analysis.config as config_module
    import frost_analysis.dataset as dataset_module
    from frost_analysis.dataset import aggregate_original
    from frost_analysis.dataset_io import write_json
