...
SCRIPT="$PROJECT_ROOT/scripts/upload_cycle_images.sh"
...
test_different_size_cloud_zip_is_reuploaded() {
    new_fixture
    install_fake_zip
    install_fake_rclone
    ...
    grep -Fq "rclone copyto" "$log" || fail "different-size cloud ZIP must be replaced"
    assert_file_missing "$images/frost_cycle_000003"
    assert_file_missing "$tmp/frost_cycle_000003.zip"
    [[ "$(/usr/bin/stat -f '%z' "$cloud/frost_cycle_000003.zip")" == 32 ]]
}
