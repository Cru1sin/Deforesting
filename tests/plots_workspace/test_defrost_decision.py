...
def _module():
    path = Path("plots/defrost_decision.py")
        ...
        [
            {
                ...
                "candidate_defrost_time": stable + pd.Timedelta(minutes=minute),
...
def test_water_reference_curve_uses_its_own_selected_defrost_time() -> None:
    module = _module()
    table = _table("v1")
...
def test_cost_curve_selected_marker_uses_true_regret() -> None:
    ...
    selected = table["cycle_name"].eq("cycle_003") & table["candidate_defrost_time"].eq(
...
def test_cost_curve_rgb_fetches_only_missing_optimal_front_members(
    ...
    import dataset_tools.cloud_images as cloud_images
...
def test_policy_loader_uses_authoritative_knee_and_preserves_abstention(
    ...
    run.mkdir()
    (run / "recipe.json").write_text(
        json.dumps({"base_method": "cop_heating_rate_pareto_knee", "heat_basis": "water"}),
        encoding="utf-8",
    )
    rows = []
    ...
    table = module._load_result_tables([run], Loader())["cop_heating_rate_pareto_knee"]
...
def test_loader_prefers_the_common_selection_contract(tmp_path: Path) -> None:
    ...
    selected_defrost_time = start + pd.Timedelta(minutes=11)
    pd.DataFrame(
        {
            "cycle_name": "cycle_003",
            "candidate_defrost_time": [start + pd.Timedelta(minutes=10), selected_defrost_time],
            "selected": [False, True],
            "selected_defrost_time": [selected_defrost_time, selected_defrost_time],
            "selected_point_in_training_domain": [False, False],
        }
    ...
    assert table["t_star"].eq(selected_defrost_time).all()
    assert not table["t_star_model_supported"].any()


@pytest.mark.parametrize(
    ("knees", "selected_defrost_times"),
    [
...
def test_policy_loader_rejects_inconsistent_stored_selection(
    tmp_path: Path,
    knees: list[bool],
    selected_defrost_times: list[pd.Timestamp | pd.NaT],
) -> None:
    module = _module()
    start = pd.Timestamp("2026-01-01")
    run = tmp_path / "policy"
    ...
    pd.DataFrame(
        {
            ...
            "pareto_knee": knees,
            "selected_defrost_time": selected_defrost_times,
            "C_model_supported": True,
            "H_model_supported": True,
        }
    ).to_csv(run / "cost.csv", index=False)

    with pytest.raises(ValueError, match="invalid COP–heating-rate Pareto selection"):
...
def test_cycle_sets_pass_policy_curve_only_to_shared_parallel_renderer(
    ...
    policy = historical.assign(algorithm="cop_heating_rate_pareto_knee", C=1.0, H=2.0, O=3.0)
    ...
    module._render_cycle_sets(
        {"v1": historical, "cop_heating_rate_pareto_knee": policy},
        Loader(),
    ...
    assert seen["cop_heating_rate_pareto_knee"] is not None
    assert seen["cop_heating_rate_pareto_knee"]["candidate_defrost_time"].equals(
        policy["candidate_defrost_time"]
    )
...
def test_parallel_publication_calls_shared_panels_when_rgb_is_unavailable(
    ...
    policy = pd.DataFrame(
        {
            "candidate_defrost_time": pd.date_range(start, periods=3, freq="min"),
            "cycle_cop": [2.0, 2.1, 2.05],
            "cycle_heating_rate_kw": [4.0, 3.9, 4.1],
            "cycle_evaporator_capacity_kw": [3.0, 3.1, 3.2],
...
def test_decision_comparison_labels_unselected_cycle_as_not_selected() -> None:
    module = _module()
    table = _table("cop_heating_rate_pareto_knee").assign(
        cycle_start=pd.Timestamp("2025-12-31 23:55:00"),
        t_star=pd.NaT,
        t_star_model_supported=pd.NA,
    )
    ...
    figure = module._comparison_figure(
        {"cop_heating_rate_pareto_knee": table}, ("cop_heating_rate_pareto_knee",)
    )

    labels = {collection.get_label() for collection in figure.axes[0].collections}
    assert "COP–heating-rate Pareto selection (no selected point)" in labels
    assert not any("diagnostic minimum" in label for label in labels)
    plt.close(figure)


def test_pareto_panel_labels_front_times() -> None:
    ...
    values = pd.DataFrame(
        {
            ...
            "cycle_cop": [2.0, 2.1],
            "cycle_heating_rate_kw": [4.1, 4.0],
            "cycle_evaporator_capacity_kw": [2.5, 2.4],
            "cycle_cop_eligible": True,
            "cycle_heating_rate_kw_eligible": True,
            "is_cop_heating_rate_pareto_point": True,
            "is_selected_pareto_point": [False, True],
        }
    )
