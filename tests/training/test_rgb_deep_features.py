...
import frost_analysis.training.features as rgb_deep_features
...
def test_add_deep_features_streams_selected_representations(monkeypatch) -> None:
    loaded = []
    monkeypatch.setattr(
        rgb_deep_features,
        "load_frozen_extractor",
        lambda name: (loaded.append(name) or object(), object()),
    )
    monkeypatch.setattr(
        rgb_deep_features,
        "extract_embeddings",
        lambda paths, model, transform, **kwargs: np.ones((len(paths), 1)),
    )

    result = add_deep_features(
        pd.DataFrame({"file_name": ["a.jpg", "b.jpg"]}),
    ...
    assert loaded == ["dinov2", "efficientnet"]
    assert result[["dinov2_000", "efficientnet_000"]].eq(1).all().all()
