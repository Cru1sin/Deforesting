...
def test_analysis_script_writes_curves_tables_figures_and_report(tmp_path: Path) -> None:
    ...
    assert (output_dir / "source_data" / "smoothed_curves.parquet").is_file()
    assert (output_dir / "source_data" / "method_metrics.csv").is_file()
    assert (output_dir / "source_data" / "cycle_recommendations.csv").is_file()
    assert (output_dir / "source_data" / "cost_method_ranking.csv").is_file()
    assert (output_dir / "figures" / "cycles" / "cycle_test.png").is_file()
    assert (output_dir / "figures" / "method_overview.png").is_file()
