...
from defrost_decision.baselines.unit_heat_inverse_cop_v1 import validate_recipe
from defrost_decision.selection_results import build_historical_cost_curve, cycle_ratio
...
def test_component_override_requires_a_named_variant() -> None:
    ...
    assert validate_recipe(recipe)["run_name"] == "trial"
    assert validate_recipe(recipe)["label_eligible"] is False

    normalized = validate_recipe(
        _recipe(run_name="trial", transition_heat_model="linear_qprep_plus_signed_quadratic_qd")
    )
...
def test_noncanonical_measurement_protocol_requires_label_ineligible_variant(
    ...
    checked = validate_recipe(_recipe(run_name="strict_audit", **protocol))
...
def test_variant_still_rejects_unknown_or_incompatible_components() -> None:
    with pytest.raises(ValueError, match="transition heat model"):
        validate_recipe(_recipe(run_name="trial", transition_heat_model="experimental"))
    with pytest.raises(ValueError, match="heat basis.*heating heat model"):
        validate_recipe(_recipe(run_name="trial", heat_basis="water"))
...
def test_build_historical_cost_curve_uses_joint_support_positive_heat_and_argmin() -> None:
    ...
    boundaries = pd.DataFrame(
        {
            ...
            "minutes_since_heating_start": [10.0, 11.0, 12.0, 13.0],
        }
    )
    eh = pd.DataFrame(
        {
            "pre_defrost_electricity_kwh": [1.0, 1.0, 2.0, 1.0],
    ...
    qh = pd.DataFrame(
        {
            "pre_defrost_heat_kwh": [1.0, 2.0, -1.0, 2.0],
    ...
    et = pd.DataFrame(
        {
            "defrost_event_electricity_kwh": [0.0, 0.0, 0.0, 0.0],
            "preparation_energy_kwh": 0.0,
            "defrost_energy_kwh": 0.0,
            "recovery_energy_kwh": 0.0,
            "defrost_event_electricity_evaluable": [True, True, True, True],
            "defrost_event_electricity_in_training_domain": [False, False, False, False],
    ...
    qt = pd.DataFrame(
        {
            ...
            "defrost_event_net_heat_evaluable": [True, True, True, True],
            "defrost_event_net_heat_in_training_domain": [False, False, False, False],
    ...
    assert result["support_rule"].eq("allow_historical_extrapolation").all()
    assert result["inverse_cop"].iloc[:2].tolist() == pytest.approx([1.0, 0.5])
    assert result["is_optimum"].tolist() == [False, True, False, False]
    assert result["is_selected"].equals(result["is_optimum"])
    assert result["selected_defrost_time"].eq(times[1]).all()
    assert result["selection_method"].eq("argmin_inverse_cop").all()
    assert result["selection_status"].eq("selected").all()
    assert result["selection_reason"].eq("historical_eligible_minimum").all()
    assert result.loc[result["is_selected"], "selection_score"].item() == 0.5
    assert not result["selected_point_in_training_domain"].any()
    ...
    assert result["run_name"].isna().all()
    assert result["label_eligible"].all()


def test_build_historical_cost_curve_rejects_positive_defrost_heat() -> None:
    boundary = pd.DataFrame(
        {"cycle_name": ["cycle_a"], "candidate_defrost_time": [pd.Timestamp("2026-01-01")]}
    )
    eh = pd.DataFrame({"pre_defrost_electricity_kwh": [1.0], "heating_valid": [True]})
    qh = pd.DataFrame({"pre_defrost_heat_kwh": [2.0]})
    et = pd.DataFrame(
        {"defrost_event_electricity_kwh": [0.0], "defrost_event_electricity_evaluable": [True]}
    )
    qt = pd.DataFrame(
        {
            "defrost_event_net_heat_kwh": [0.1],
...
def test_cycle_ratio_names_totals_and_masks_nonfinite_or_zero_heat() -> None:
    curve = pd.DataFrame(
        {
            "pre_defrost_electricity_kwh": [1.0, 1.0, float("inf")],
            "defrost_event_electricity_kwh": [0.5, 0.5, 0.5],
            "pre_defrost_heat_kwh": [2.0, 0.0, 2.0],
            "defrost_event_net_heat_kwh": [1.0, 0.0, 1.0],
        }
    )
