...
from frost_analysis.defrost_cost import optimize_cycle_cop_cost
...
def analyze(dataset: Path, policy_path: Path, output: Path) -> None:
    ...
    curve, optimum = optimize_cycle_cop_cost(
        candidates,
        defrost_recovery_electricity_kwh=float(policy["mean_ticket_electricity_kwh"]),
        required_end_time=actual,
        near_optimal_fraction=0.01,
    )
    curve.insert(0, "cycle_name", OOD_CYCLE)
    curve["relative_regret"] = curve["inverse_cop"] / float(
        optimum["inverse_cop"]
    ) - 1
