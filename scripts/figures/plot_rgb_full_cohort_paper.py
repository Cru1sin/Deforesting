...
REPRESENTATIONS = ["handcrafted", "dinov2", "efficientnet", "mobilenet_v3_small", "repvit_m0_9", "convnext_tiny", "dinov3", "siglip2"]
MODELS = ["logistic", "random_forest", "rbf_svm", "hist_gradient_boosting", "mlp", "window_logistic"]
CAMERAS = ["all", "top", "top_close", "left", "left_close", "front", "extreme"]
REP_LABELS = {
    "handcrafted": "Handcrafted", "dinov2": "DINOv2", "efficientnet": "EfficientNet",
    "mobilenet_v3_small": "MobileNetV3-small", "repvit_m0_9": "RepViT-M0.9",
    "convnext_tiny": "ConvNeXt-Tiny", "dinov3": "DINOv3", "siglip2": "SigLIP2",
}
MODEL_LABELS = {
    "logistic": "Logistic†", "random_forest": "Random forest", "rbf_svm": "RBF SVM",
    "hist_gradient_boosting": "Hist. gradient boosting", "mlp": "MLP†", "window_logistic": "Window logistic",
}
CAMERA_LABELS = {
    "all": "All views", "top": "Top", "top_close": "Top close", "left": "Left",
    "left_close": "Left close", "front": "Front", "extreme": "Extreme",
}
WARNING_MODELS = {"logistic", "mlp"}
METRIC_LABELS = {"accuracy": "Accuracy", "balanced_accuracy": "Balanced accuracy", "macro_f1": "Macro-F1"}
CLASS_LABELS = {0: "Before", 1: "Within", 2: "After"}
BEST = {"representation": "mobilenet_v3_small", "model": "random_forest", "camera_group": "all", "modality": "rgb", "regret_threshold": 0.01}
...
def _scores(target: np.ndarray, predicted: np.ndarray) -> dict[str, float]:
    ...
    return {
        "accuracy": float(np.mean(target == predicted)), "balanced_accuracy": float(np.mean(recalls)),
        "macro_f1": float(np.mean(f1s)), **{f"recall_{label}": float(recalls[label]) for label in (0, 1, 2)},
    }
...
def _matrix_sources(summary: pd.DataFrame, manifest: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in rows.itertuples(index=False):
        out.append({
            ...
            "representation_label": REP_LABELS[row.representation], "model_label": MODEL_LABELS[row.model],
            "macro_f1": row.estimate, "lower_95": row.lower, "upper_95": row.upper,
            "evaluable_experiment_count": row.evaluable_experiment_count,
            "convergence_warning_noted": row.model in WARNING_MODELS,
            "warning_note": "A small number of ConvergenceWarning events occurred." if row.model in WARNING_MODELS else "",
        })
...
def _best_prediction_sources(predictions: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for camera in CAMERAS:
        ...
        for experiment, group in subset.groupby("experiment_id"):
            if not group["fold_evaluable"].all() or group["predicted_target"].isna().any() or set(group["target"]) != {0, 1, 2}:
                continue
    ...
    for camera_index, camera in enumerate(CAMERAS):
        ...
        for metric_index, metric in enumerate(METRIC_LABELS):
            estimate, lower, upper = _bootstrap(group[metric], 2100 + camera_index * 10 + metric_index)
            metric_rows.append({
                "camera": camera, "camera_label": CAMERA_LABELS[camera], "metric": metric,
                "metric_label": METRIC_LABELS[metric], "estimate": estimate, "lower_95": lower, "upper_95": upper,
                "evaluable_experiment_count": len(group),
            })
        for label in (0, 1, 2):
            estimate, lower, upper = _bootstrap(group[f"recall_{label}"], 3100 + camera_index * 10 + label)
            recall_rows.append({
                "camera": camera, "camera_label": CAMERA_LABELS[camera], "class_id": label,
                "class_label": CLASS_LABELS[label], "recall": estimate, "lower_95": lower, "upper_95": upper,
                "evaluable_experiment_count": len(group),
            })
...
def _plot_matrix(source: pd.DataFrame, output: Path) -> None:
    matrix = source.pivot(index="representation", columns="model", values="macro_f1").reindex(index=REPRESENTATIONS, columns=MODELS)
    ...
    ax.set_xticks(np.arange(len(MODELS)), [MODEL_LABELS[x] for x in MODELS], rotation=30, ha="right")
    ax.set_yticks(np.arange(len(REPRESENTATIONS)), [REP_LABELS[x] for x in REPRESENTATIONS])
    for row in range(matrix.shape[0]):
        for column in range(matrix.shape[1]):
            value = matrix.iloc[row, column]
            count = source.loc[(source.representation.eq(REPRESENTATIONS[row])) & (source.model.eq(MODELS[column])), "evaluable_experiment_count"].iloc[0]
            ax.text(column, row, f"{value:.3f}\nn={int(count)}", ha="center", va="center", fontsize=5.8, color="white" if value > 0.78 else "#303030")
    ...
    ax.text(0, -0.26, "† A small number of ConvergenceWarning events occurred; these models are not used for the primary conclusion.",
            transform=ax.transAxes, ha="left", va="top", fontsize=6, color="#606060")
...
def _plot_view_metrics(source: pd.DataFrame, output: Path) -> None:
    ...
    for metric in METRIC_LABELS:
        ...
        ax.errorbar(rows["estimate"], y + offsets[metric], xerr=[rows["estimate"] - rows["lower_95"], rows["upper_95"] - rows["estimate"]],
                    fmt="o", ms=4, capsize=2, elinewidth=1, color=colors[metric], label=METRIC_LABELS[metric])
    counts = source.drop_duplicates("camera").set_index("camera").reindex(CAMERAS)["evaluable_experiment_count"]
    ax.set_yticks(y, [f"{CAMERA_LABELS[camera]}  (n={int(counts.loc[camera])})" for camera in CAMERAS])
    ax.invert_yaxis()
...
def _plot_recall(source: pd.DataFrame, output: Path) -> None:
    matrix = source.pivot(index="camera", columns="class_id", values="recall").reindex(index=CAMERAS, columns=[0, 1, 2])
    ...
    ax.set_yticks(np.arange(len(CAMERAS)), [f"{CAMERA_LABELS[c]}  (n={int(counts.loc[c])})" for c in CAMERAS])
    for row in range(matrix.shape[0]):
        for column in range(matrix.shape[1]):
            value = matrix.iloc[row, column]
            ax.text(column, row, f"{value:.3f}", ha="center", va="center", fontsize=6.5, color="white" if value > 0.80 else "#303030")
    cbar = fig.colorbar(image, ax=ax, fraction=0.035, pad=0.025)
...
def _plot_sensor_fusion_f1(source: pd.DataFrame, output: Path) -> None:
    ...
    fig.text(0.34, 0.03, "All sensors includes COP, power and capacity residual;\nassociation is not vision-only.",
        fontsize=5.5,
        color="#606060",
    )
...
def _plot_sensor_fusion_delta(source: pd.DataFrame, output: Path) -> None:
    ...
    ax.set_yticks(
        y,
        [
            f"{FUSION_METRICS[row.metric]} · {FUSION_LABELS[row.comparison.removesuffix('_minus_rgb')].removeprefix('RGB + ').removesuffix(' sensors')}"
    ...
    fig.text(0.36, 0.03, "All sensors includes COP, power and capacity residual;\nassociation is not vision-only.",
        fontsize=5.5,
        color="#606060",
    )
...
def _render_sensor_fusion(results: Path, output: Path, summary: pd.DataFrame) -> None:
    ...
    source = summary.loc[
        summary["metric"].isin(FUSION_METRICS)
        & summary["modality"].isin(FUSION_LABELS)
    ].copy()
    ...
    source["reviewer_note"] = source["modality"].map(
        {"rgb_all_sensor": "Includes COP, compressor power and evaporator-capacity baseline residual; not vision-only."}
    ).fillna("")
    ...
    delta["reviewer_note"] = delta["comparison"].map(
        {"rgb_all_sensor_minus_rgb": "Includes COP, compressor power and evaporator-capacity baseline residual; not vision-only."}
    ).fillna("")
...
def render(results: Path, output: Path) -> None:
    ...
    expected = summary.loc[
        summary["metric"].eq("macro_f1") & summary["camera_group"].eq("all")
        & summary["modality"].eq("rgb") & summary["regret_threshold"].eq(0.01)
    ]
...
def main() -> None:
    ...
    parser.add_argument("--results", type=Path, default=Path("report/03_RGB标签与模型/全量模态比较"))
