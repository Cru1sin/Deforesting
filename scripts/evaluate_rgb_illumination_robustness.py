...
def plot_summary(summary: pd.DataFrame, output: Path) -> None:
    colors = {"dinov2": "#4C78A8", "efficientnet": "#F58518"}
...
def main() -> None:
    ...
    base_extractors = load_frozen_extractors(list(DEEP_REPRESENTATIONS))
    condition_frames = {"native": sampled.copy()}
    metadata = sampled.drop(
        columns=[
            column
            for column in sampled
            if column.startswith("dinov2_") or column.startswith("efficientnet_")
        ]
    )
    ...
    metrics = evaluate_conditions(cohort, condition_frames, args.camera_groups)
