...
from train_rgb_smoke_models import BinaryResNet50, ImageRows, _transforms
...
def export_latents(run_dir: Path, dataset: Path, batch_size: int, workers: int) -> Path:
    ...
    model.eval()
    evaluation_transform = _transforms()[1]
    loader = DataLoader(
...
def structure_check(run_dir: Path, dataset: Path) -> None:
    ...
    images, _ = next(iter(DataLoader(ImageRows(manifest, _transforms()[1]), batch_size=2)))
    with torch.no_grad():
