...
本报告在77个循环上比较原始制热量、根据水流量和进出水温差独立计算的水侧制热量参考，以及九种平滑结果。下面按使用目的分别给出结论，不把不同目标混成一个“最好方法”。

**普通离线分析。** 按每个循环在曲线形状、降噪、能量、短时响应和滞后五项指标上的排名求和，`wavelet_offline` 的跨循环平均秩和最低（9.74，数值越低表示综合排名越靠前），因此小波方法用作总体保真度对照。

**除霜成本计算。** 先要求极端跳变至少降低10%，再比较供热量不足的累积误差、能量误差、短时响应和水侧曲线形状时，`savgol_70s` 的平均秩和最低（14.73）。如后续成本函数必须统一使用一种离线平滑，选择约70秒 Savitzky–Golay；小波方法作为敏感性对照。不为不同循环分别选择最优方法。

**实时控制。** 75/77个循环推荐时间常数为30秒的指数加权移动平均（EWMA），其余2个推荐先用30秒因果中值滤波去除尖峰，再接30秒 EWMA。居中中值、Savitzky–Golay、小波和单调回归使用未来样本或完整阶段信息，不能用于实时控制。

**系统性偏差。** 各种平滑结果相对水侧制热量参考的中位偏差约为−0.372 kW，且方法之间十分接近。这说明平滑可以抑制随机波动和尖峰，但不能消除两种制热量计算之间的系统性偏差。
...
- 水侧制热量参考为 `1.161 × water_flow[m³/h] × (T_out−T_in)[K]` kW。它可用于检查曲线形状和偏差，但温度通道可能与控制器计算共享，因此不能称为完全独立的金标准。
...
- **水侧去偏 RMSE**：先取常数偏差 $b=\operatorname{median}(s_i-w_i)$，再算 $\sqrt{N^{-1}\sum_i(s_i-w_i-b)^2}$。越小表示曲线形状越接近水侧制热量参考；因为水侧与控制器信号可能共享传感器，它只是参考量，不是独立真值。
