...
def add_selected_time_fields(
    ...
    *,
    ...
    """Expose the common selected-time fields without removing method-specific fields."""
    result = curve.copy()
    result["is_selected"] = selected.fillna(False).astype(bool)
    result["selected_defrost_time"] = pd.NaT
    ...
    result["selected_point_in_training_domain"] = pd.NA
    groups = (
    ...
    for positions in groups:
        chosen = result.index.intersection(positions)[result.loc[positions, "is_selected"]]
        if chosen.empty:
            continue
        index = chosen[0]
...
def build_historical_cost_curve(  # noqa: C901
    ...
    """Apply the shared V1/V2.5 allow_historical_extrapolation selection rule."""
    checked = dict(recipe)
    lengths = {
        len(part)
    ...
    et_evaluable = curve["defrost_event_electricity_evaluable"].fillna(False)
    ...
    curve["is_censored"] = False
    return add_selected_time_fields(
        curve,
        curve["is_optimum"],
        method="argmin_inverse_cop",
