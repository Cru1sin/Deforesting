...
Defrost initiation in air-source heat pumps is an economic decision: continued heating avoids a defrost interruption but progressively loses useful heat as frost accumulates. Historical records, however, contain only the action selected by the installed controller and provide no decision label for the preceding frost images. Here we introduce a retrospective framework that converts unsmoothed, approximately 1-s water-side and electrical measurements into a policy-conditional renewal-cost surface and maps its pointwise relative regret to timestamped RGB images. Among 77 catalogued cycles, 47 had complete candidate domains and 37 of these exhibited an interior empirical minimum. The median minimum preceded the observed defrost action by 58.8 min, but the median envelope spanning candidates within 1% of the minimum was 33.0 min, showing that a precise timestamp was often weakly identified. The resulting image dataset comprised 105,178 metadata rows, including 57,213 images within cost-valid candidate domains. Five-model development favoured a high-confidence binary task that excludes low-regret images over direct pre/near/post classification. **[FULL-COHORT RESULT: insert experiment-level RGB, retrospective time-only and RGB-plus-time estimates and 95% intervals.]** These results establish an auditable bridge from system cost to ambiguity-aware visual supervision. They do not establish a causal global optimum, prospective energy savings, occupant comfort improvement or online deployment.
...
**[FULL-COHORT RESULT BLOCK — populate only after all eligible cycles complete.]** Report, for each locked camera group, the macro-average across held-out experiments for balanced accuracy, macro-F1, AUROC and class-balanced misclassification regret. State the RGB-minus-time and RGB-plus-time-minus-time paired mean differences with experiment-level 95% bootstrap intervals. Explicitly report the number of experiments, cycles and sampled images. The primary claim is allowed only if RGB provides a consistent increment over the time-only control; otherwise report that the visual signal did not demonstrate incremental decision value.
...
**[FULL-COHORT FAILURE BLOCK.]** Report the distribution of held-out experiment scores and the worst experiments for balanced accuracy and class-balanced misclassification regret. Describe errors at high relative regret separately from errors near the excluded 1% boundary. Do not pool frames to obtain confidence intervals. Because historical test outputs were inspected during development, describe the complete leave-one-experiment-out analysis as developmental cross-validation rather than a blind test.
...
Experiment-level isolation and the time-only control are equally important. Adjacent images are not independent observations, and a high frame-level score can reflect experiment identity, camera exposure or elapsed time. A visual model becomes decision-informative only if its held-out performance and error regret improve over an identical time-only evaluation. **[Update interpretation after the full-cohort paired interval is known.]**
...
- All `FULL-COHORT` fields are intentional blockers and must be populated from final experiment-level outputs, never from the interim report.
