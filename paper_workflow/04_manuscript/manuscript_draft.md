...
**Figure 1 | Unsmoothed operational data define policy-conditional empirical defrost minima.** **a,** Representative approximately 1-s water-side heating capacity (grey), clean reference (blue), earliest empirical minimum (orange) and observed defrost action (dashed). **b,** Renewal-average cost across the valid candidate domain for the same cycle; the orange line marks its earliest minimum. **c,** Empirical optimum versus observed defrost timing for 47 complete cycles. Blue and grey denote interior and right-boundary minima; the diagonal denotes equality. **d,** Distribution of minutes by which the empirical minimum preceded the observed action. The box shows the interquartile range and median; points are cycles. All minima are conditional on the observed fixed-duration ticket and offline clean-reference contract.

**Figure 2 | Pointwise regret converts cost valleys into ambiguity-aware image labels.** **a,** Representative renewal-cost curve labelled pre-optimal, near-optimal or post-optimal at 1% relative regret; classification uses each candidate's regret and does not fill gaps between disconnected low-regret candidates. **b,** Per-cycle envelope widths at 1%, 2%, 5% and 10%; black bars are medians. **c,** Absolute fraction of candidate-domain images retained after excluding the near-optimal class. **d,** Experiments and cycles in the original model-development split. Whole experiments, not frames, define partitions.

**Figure 3 | Incremental visual information depends on camera position and exclusion threshold.** **a,** Experiment-macro balanced accuracy for RGB, retrospective time and RGB plus time at the pre-locked 1% threshold across nine camera groups. **b,** Paired balanced-accuracy differences relative to time alone; the dashed line denotes zero. **c,** All-view performance and absolute eligible-image coverage at 1%, 2%, 5% and 10% exclusion thresholds. `n` gives experiments with both classes and hence evaluable balanced accuracy. Points are experiment-level means; error bars are percentile 95% intervals from 5,000 experiment bootstrap resamples. The normalized time-progress control uses the future observed boundary and is a retrospective diagnostic, not an online feature. Front-camera comparisons are exploratory.

**Figure 4 | Experiment and cycle failures bound the visual claim.** **a,** Balanced accuracy for all-view RGB, retrospective time and RGB plus time in each held-out experiment at 1% regret. **b,** Corresponding class-balanced misclassification regret; larger values assign more cost to errors far from the empirical optimum. **c,** Ten cycles with the largest mean RGB misclassification regret; annotations give image-level error rates. All results are developmental leave-one-experiment-out estimates, not a blind confirmatory test.
