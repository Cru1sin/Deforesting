...
def _predict_additional_outcome(
    ...
    *,
    ...
    prediction = predict_with_heldout_event_model(model, candidates, experiment_id)
    threshold = float(model["folds"][experiment_id]["support_threshold"])
    ...
    candidates[f"{field_name}_in_training_domain"] = (
        prediction["support_distance"].le(threshold).to_numpy()
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    experiments = set.intersection(*(set(model_set[name]["folds"]) for name in required))
    ...
    if args.dry_run:
        print(
            ...
            f"step={args.candidate_step_seconds}s, workers={args.workers}, output={run}"
        )
        return 0
